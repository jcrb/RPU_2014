...
load("../rpu2014d0112_c.Rda") # d14 = données 2014 corrigées
dx <- d14                     # dx est le terme générique pour désigner le dataframe de l'année courante
...
partie centrale du rapport dans laquelle tous les résultats d’activité sont présentés dans le déroulement d’une trame.
...
n2013 <- 344073
```
- nombre de passages en `r anc`: `r n` soit en moyenne `r round(mean.rpu.jour, 0)` par jour.
...
les analyses par filière : focus sur une sous déclinaison de l’activité (pathologies traceuses ou traits caractéristiques de la patientèle)