...
```{}
d12 <- dx[as.Date(dx$ENTREE) >= "2015-12-01" & as.Date(dx$ENTREE) < "2016-01-01",]
min(as.Date(d12$ENTREE))
max(as.Date(d12$ENTREE))

d12 <- normalise(d12)
save(d12, file="rpu2015d12_provisoire.Rda")
rm(dx)
...
d15 <- d15[as.Date(d15$ENTREE) < "2015-12-01",]
...
t <- t[, c("janvier","février","mars", "avril","mai","juin","juillet", "août", "septembre", "octobre", "novembre")]
write.csv2(t, file = "rpu_par_mois.xls")
```

# Mois courant: projection du nb de RPU à mois constant de 30 jours. 
n.jours <- as.numeric(max(as.Date(d11$ENTREE))-min(as.Date(d11$ENTREE)) + 1)
n.rpu.mois.constant <- nrow(d11) * 30 / n.jours
n.rpu.mois.constant
...
t <- tapply(as.Date(d15$ENTREE), month(as.Date(d15$ENTREE), label = TRUE), length)
names(t) <- c("janvier","février","mars", "avril","mai","juin","juillet", "août", "septembre","octobre", "novembre")
...
n.j <- as.integer(diff(seq(as.Date("2015-01-01"), as.Date("2015-12-01"), by = "month")))
...
write.csv(rpu.jour, file = "rpu.jour_26-11-2015.csv") 
...
rpu <- read.csv("rpu.jour_26-11-2015.csv")
names(rpu)[1] <- "date"
names(rpu)[2] <- "3Fr"
