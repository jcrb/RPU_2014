...
```{}
d08 <- dx[as.Date(dx$ENTREE) >= "2014-08-01" & as.Date(dx$ENTREE) < "2014-09-01",]
max(as.Date(d08$ENTREE))
min(as.Date(d08$ENTREE))
d08 <- normalise(d08)
save(d08, file="rpu2014d08_provisoire.Rda")
rm(dx)

load("rpu2014d0107.Rda")

dx <- rbind(dx, d08)
min(as.Date(dx$ENTREE))
max(as.Date(dx$ENTREE))

save(dx, file="rpu2014d0108_provisoire.Rda")

table(as.Date(d08$ENTREE), d08$FINESS)
```
