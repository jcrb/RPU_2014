...
```{}
d06 <- dx[as.Date(dx$ENTREE) >= "2015-06-01" & as.Date(dx$ENTREE) < "2015-07-01",]
max(as.Date(d06$ENTREE))
min(as.Date(d06$ENTREE))
d06 <- normalise(d06)
save(d06, file="rpu2015d06_provisoire.Rda")
rm(dx)
...
dx <- rbind(d01, d02, d03, d04, d05, d06)
min(as.Date(dx$ENTREE))
max(as.Date(dx$ENTREE))
...
rpu.par.jour(d05)
>>>>>>> dd48535fad39ab89dd637c24ae1ccb99e8e3ce0e
```
...
------------------------
On forme une table en croisant FINESS et ENTREE pour constituer le fichier __exaustivite_rpu.csv__, ensemble des RPU de l'année par FINESS.
...
plot(xts.a, las = 2, main = "Activité des SU d'Alsace en 2015", ylab = "nombre de RPU créés par jour", ylim = c(950,1650))
mean2014 <- 1141.734
sd2014 <- 154.7858
...
# résumé du pourcentage de diagnostic non codés
summary(p.dp.non.code)
# plot
plot(p.dp.non.code, type = "l", ylim = c(0,1), ylab = "% de DP non codés", xlab = "2015 (jours)", main = "Pourcetage de RPU dont le DP est manquant")
...
# on transforme la matrice des % de DP non codés en dataframe du % de DP non codés par jour et par FINESS
...
plot(p.codage, col="blue", ylim = c(0,1.1), ylab = "% de DP codés", main = "Pourcentages de RPU dont le DP est renseigné")
