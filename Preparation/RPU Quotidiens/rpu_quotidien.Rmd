...
```{}
d01 <- dx[as.Date(dx$ENTREE) >= "2014-01-01" & as.Date(dx$ENTREE) < "2015-01-31",]
max(as.Date(d01$ENTREE))
min(as.Date(d01$ENTREE))
d01 <- normalise(d01)
save(d01, file="rpu2015d01_provisoire.Rda")
rm(dx)

# uniquement le premier mois
save(d01, file="rpu2015d0112_provisoire.Rda")

load("rpu2015d0112_provisoire.Rda")

dx <- rbind(dx, d12)
min(as.Date(dx$ENTREE))
max(as.Date(dx$ENTREE))

save(dx, file="rpu2015d0112_provisoire.Rda")

table(as.Date(d01$ENTREE), d01$FINESS)
```
...
rpu <- table(as.Date(dx$ENTREE), dx$FINESS)
```
...
plot.xts2(x, ylab="nombre de RPU", xlab=paste0("Année ", year(Sys.Date())), main="Activité des SU d'Alsace en nombre de RPU", col="blue", minor.ticks = FALSE)
...
```{}
d <- d[as.Date(d$ENTREE) > max(as.Date(d01$ENTREE)),]
d01.p <- rbind(d01, d)
max(as.Date(d01.p$ENTREE))

```
et l'afficher sous forme de série temporelle
```{}
a <-tapply(as.Date(d01.p$ENTREE), as.Date(d01.p$ENTREE), length)
xts.a <- xts(a, order.by = unique(as.Date(d01.p$ENTREE)))
plot(xts.a, las = 2)
abline(h = 1141.734, col = "red") # nb moyende RPU en 2014
abline(h = 1141.734 + 154.7858, col = "red", lty = 2) # 1 écrt-type
abline(h = 1141.734 + 154.7858 * 2, col = "red", lty = 2) # 2 écart-type
