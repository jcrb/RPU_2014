...
```{}
d10 <- dx[as.Date(dx$ENTREE) >= "2015-10-01" & as.Date(dx$ENTREE) < "2015-11-01",]
min(as.Date(d10$ENTREE))
max(as.Date(d10$ENTREE))

d10 <- normalise(d10)
save(d10, file="rpu2015d10_provisoire.Rda")
rm(dx)

# si le mois est complet:
# save(d10, file="rpu2015d10.Rda")
# rm(rpu2015d10_provisoire.Rda)
...
d15 <- d15[as.Date(d15$ENTREE) < "2015-10-01",]
...
# nb de RPU à mois constant de 30 jours
n.jours <- as.numeric(max(as.Date(d10$ENTREE))-min(as.Date(d10$ENTREE)) + 1)
n.rpu.mois.constant <- nrow(d10) * 30 / n.jours

Total par mois et par mois constants
```{}
t <- tapply(as.Date(d15$ENTREE), months(as.Date(d15$ENTREE)), length)
t <- t[, c("janvier","février","mars", "avril","mai","juin","juillet", "août", "septembre")]
t
...
n.j <- as.integer(diff(seq(as.Date("2015-01-01"), as.Date("2015-10-01"), by = "month")))
...
#### Nombre de jours attendus pour l'année:
...
write.csv(rpu.jour, file = "rpu.jour_31-10-2015.csv") 
