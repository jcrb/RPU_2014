...
```{}
d08 <- dx[as.Date(dx$ENTREE) >= "2015-08-01" & as.Date(dx$ENTREE) < "2015-09-01",]
min(as.Date(d08$ENTREE))
max(as.Date(d08$ENTREE))

d08 <- normalise(d08)
save(d08, file="rpu2015d08_provisoire.Rda")
rm(dx)
...
t <- t[, c("janvier","février","mars", "avril","mai","juin","juillet")]
...
```{}
rpu.jour <- rpu.par.jour(d15) # matrice des RPU depuis le début année
write.csv(rpu.jour, file = "rpu.jour_31-07-2015.csv") # sauvegarde de la matrice
rpu.manquant <- function(x){length(which(x == 0))} # fonction qui compte le nombre de rpu = 0 dans le vecteur x
apply(rpu.jour, 2, rpu.manquant) # applique la fonction à la matrice rpu.jour
...
library(xts)
rpu <- read.csv("rpu.jour_31-07-2015.csv")
