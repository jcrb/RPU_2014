...
pie(table(dpccm$SEXE), col = c("red","blue"), labels = leg, main = "CCOM - Répartition par sexe")


# Répartition des codes CIM 10
barplot(sort(table(dpccm$DP)), las = 2)
...
Pétards - Extension à tous les sos mains
----------------------------------------
```{r}
sos <- d15.p[d15.p$FINESS %in% c("Ccm","Dts","Ros") ,]
dpsos <- sos[!is.na(sos$DP),]
p <- dpsos[str_detect(dpsos$DP, pattern) == TRUE,]
nrow(p)
```

Lésions de la main - Extension à tous les sos mains
---------------------------------------------------

```{r}
tb <- data.frame(table(dpsos$DP))
a <- merge(tb, cim10, by.x = "Var1", by.y = "CODE", all.x = TRUE)
...
---------
```{r}
ros <- dpsos[dpsos$FINESS == "Ros",]
tb <- data.frame(table(ros$DP))
a <- merge(tb, cim10, by.x = "Var1", by.y = "CODE", all.x = TRUE)
a
```
4 diagnostics dont 2 aberrants...

Diaconat Strasbourg
-------------------
```{r}
dts  <- dpsos[dpsos$FINESS == "Dts",]
tb <- data.frame(table(dts$DP))
a <- merge(tb, cim10, by.x = "Var1", by.y = "CODE", all.x = TRUE)

```
Diagnostics imprécis: douleur aigue, B99+1 (Syndrome infectieux sans cause trouvée)
