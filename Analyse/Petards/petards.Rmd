...
pet.dp <- pet[!is.na(pet$DP) & pet$DP %in% c("S60", "S61", "S62", "S63", "S64", "S65", "S66", "S67", "S68", "S69"), c("ENTREE","DP")]
...
pet.dp.plaie <- pet[!is.na(pet$DP) & pet$DP %in% c("S60", "S61"), c("ENTREE","DP")]
t <- tapply(as.Date(pet.dp.plaie$ENTREE), as.Date(pet.dp.plaie$ENTREE), length)
barplot(t, las=2, cex.names=0.8)
