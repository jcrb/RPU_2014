...
```{r liste_graphique, echo=FALSE}
for(i in finess){
  seuil.graphe(xts[,i], seuil(rpu[, i]), i)
}
```
...
```{r code, echo=FALSE}
# on forme un dataframeà 2 colonnes pour 1 fINESS
wis <- d15[d15$FINESS == "Wis", c("ENTREE", "DP")]
wis$ENTREE <- as.Date(wis$ENTREE)

t <- taux.codage.dp(wis)
...
boxplot(t[, 1:2])
# graphe
plot.taux.codage.dp(taux.codage.dp(d15[d15$FINESS == "Hag", c("ENTREE","DP")]), "CH Haguenau")

plot.taux.codage.dp(taux.codage.dp(d15[d15$FINESS == "Hus", c("ENTREE","DP")]), "HUS")


# for(i in finess){
#   plot.taux.codage.dp(taux.codage.dp(d15[d15$FINESS == i, c("ENTREE","DP")]), i)
# }
